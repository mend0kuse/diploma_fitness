...
export const ReviewsList = ({ reviews }: { reviews: TReview[] }) => {
    return (
        <Carousel px={'xl'} slideSize='33.333333%' slideGap='md' loop align='start' slidesToScroll={3}>
            {reviews.map(({ author, isHiddenUser, rating, text, id }) => (
                <Carousel.Slide key={id}>
                    <Card radius='md' withBorder shadow='xs' p={'md'}>
                        <Stack>
                            <Rating fractions={2} readOnly defaultValue={rating} />
                            ...
                            <Text c={isHiddenUser ? 'gray' : 'black'} size='md'>
                                {text}
                            </Text>
                        </Stack>
                    </Card>
